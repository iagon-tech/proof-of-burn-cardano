...
testsSimple = testGroup "simple"
    ...
    , let expectedLogMsg = JSON.String "Nothing burned with given commitment" in -- TODO give from main module
      check "burn and burnedTrace 1"
        (     walletFundsChange w1 (Ada.lovelaceValueOf (-50_000_000))
        ...
        do
            ...
            void $ Emulator.waitNSlots 2
    , let expectedLogMsg = JSON.String "Nothing burned with given commitment" in -- TODO give from main module TODO Actually "Value burned with given commitment: ..."
      check "burn and burnedTrace 2"
        (     walletFundsChange w1 (Ada.lovelaceValueOf (-50_000_000))
         .&&. walletFundsChange w2 (Ada.lovelaceValueOf 0)
         .&&. walletFundsChange w3 (Ada.lovelaceValueOf 0)
          -- TODO Why not detected??
        ...
        do
            ...
            void $ Emulator.waitNSlots 2
    , let expectedLogMsg = JSON.String "Nothing burned with given commitment" in -- TODO give from main module TODO Actually "Value burned with given commitment: ..."
      check "burn, burnedTrace and redeem"
        (      walletFundsChange w1 (Ada.lovelaceValueOf (-50_000_000))
          .&&. walletFundsChange w2 (Ada.lovelaceValueOf 0)
          .&&. walletFundsChange w3 (Ada.lovelaceValueOf 0)
          -- TODO Why not detected??
